...
def read_calldata(
    ...
    *,
    ...
    calldata = ops.to_sample_major(calldata, n_threads=n_threads)
...
def read_lai(
    ...
    *,
    ...
    logger.logger.info(f"Converting to sample-major.")
    lai = ops.to_sample_major(lai, n_threads=n_threads)
...
def cache_snpdat(
    ...
    *,
    ...
    # the non-negative IIDs (not excluded by dataset) should match up exactly.
...
def cache_combine_r_snpdat(
    ...
    *,
    ...
    if calldata.shape == lai.shape:
        calldata = ops.calldata_sum(calldata, n_threads=n_threads)
    n, s = calldata.shape
    assert lai.shape == (n, 2 * s), f"Ancestries (LAI) must have shape (n, 2*s) where calldata has shape (n, s), but got {lai.shape} and {calldata.shape}"

    # convert to Combine-R snpdat
...
def cache_combine_s_snpdat(
    ...
    dest: str,
    *,
    selected_ancestries: Union[np.ndarray, list] = None,
    sample_indices: np.ndarray=None,
    snp_indices: np.ndarray=None,
    sort_indices: bool =True,
    n_threads: int =1,
    ...
    the dense view has ``2 * A`` columns:
      ...
      - last  ``A`` columns: ancestry dosages per ancestry (like unphased ancestry)

    Parameters
    ----------
    pgen, pvar, psam, msp : str
        Paths to PGEN/PVAR/PSAM genotype files and LAI MSP file.
    dest : str
        Output ``.snpdat`` path.
    selected_ancestries : array-like of int, optional
        If provided, subset/reorder ancestry columns to these indices.
    sample_indices, snp_indices : np.ndarray, optional
        Subset of samples/SNPs to include.
    sort_indices : bool, optional
        If True, sort indices before reading to enable fast-paths.
    ...
    if selected_ancestries is None:
        ...
        selected = np.asarray(selected_ancestries, dtype=np.uint32)
        bytes_written, benchmark = handler.write(
            calldata=calldata,
            ancestries=lai,
            A=n_ancestries,
