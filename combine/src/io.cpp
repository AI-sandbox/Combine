#include "decl.hpp"
#include <combine_core/io/msp_reader.hpp>
...
void msp_reader(py::module_& m)
{
    ...
    py::class_<io_t>(m, "MSPReader")
        ...
        .def_property_readonly("chm", [](const io_t& s) {
            return Eigen::Map<const cb::util::rowvec_type<int32_t>>(
                s._chm.data(),
                s._chm.size()
            );
        })
        .def_property_readonly("pos", [](const io_t& s) {
            return Eigen::Map<const cb::util::rowarr_type<int32_t>>(
                s._pos.data(),
                s._pos.size() / 2,
                2
            );
        ...
        .def_property_readonly("gpos", [](const io_t& s) {
            return Eigen::Map<const cb::util::rowarr_type<double>>(
                s._gpos.data(),
                s._gpos.size() / 2,
                2
            );
        ...
        .def_property_readonly("lai", [](const io_t& s) {
            return Eigen::Map<const cb::util::rowarr_type<int8_t>>(
                s._lai.data(),
                s._chm.size(),
