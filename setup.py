...
with open("combine/__init__.py") as f:
    for line in f:
...
include_dirs = [
    os.path.join("combine", "src"),
    os.path.join("combine", "src", "include"),
    os.path.join("combine", "src", "third_party", "eigen3"),
]
extra_link_args = []
libraries = []
library_dirs = []
...
# check if micromamba environment activated (CI)
    ...
    conda_prefix = os.path.join(os.environ["MAMBA_ROOT_PREFIX"], "envs", "combine")
else:
    conda_prefix = None
...
ext_modules = [
    Pybind11Extension(
        "combine.combine_core",
        sorted(glob("combine/src/*.cpp")),  # Sort source files for reproducibility
...
# Include combine and all submodules.
# This removes setuptools warning about missing namespace packages.
packages = ["combine"] + [
    f"combine.{submod}"
    for submod in find_namespace_packages("combine")
]
...
setup(
    ...
    long_description='',
    author='James Yang, David Bonet',
    author_email='jamesyang916@gmail.com, davidbonetsole@gmail.com',
    maintainer='James Yang, David Bonet',
    maintainer_email='jamesyang916@gmail.com, davidbonetsole@gmail.com',
    packages=packages,
    package_data={
        "combine": [
            "src/**/*.hpp",
